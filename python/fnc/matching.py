...
def matching(template_extr, mask_extr, temp_dir, threshold=0.38):
	...
	print("Starting biometric verification process.")
	
	# Get the number of accounts in the database
	n_files = len(filter(listdir(temp_dir), '*.mat'))
	print("Detected .mat files : {}".format(n_files))
	if n_files == 0:
	...
	filenames = [os.path.join(os.getcwd(), "templates", "CASIA1", result_list[i][0]) for i in range(len(result_list))]
	print(filenames)
	...
	print("Results : {}".format(hm_dists))
	# Threshold and give the result ID
	print("Set threshold : {}".format(threshold))
	ind_thres = np.where(hm_dists<=0.380000)[0]
	print("Ind_threshold filter results : {}".format(ind_thres))
	# Return
	if len(ind_thres)==0:
		return 0
	else:
		...
		print("Before sort : {}".format(hm_dists))
		ind_sort = np.argsort(hm_dists)
		print("After sort : {}".format(ind_sort))
