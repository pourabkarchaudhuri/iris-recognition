...
def index():
    ...
    if request.method == 'POST':
        ...
        ids = verify.execute(uploaded_img_path)
        print(ids)
        ...
        path = [id['path'] for id in ids]
        filteredScores = [0 for id in ids]
        filteredNames = [id['id'] for id in ids]
        print(uploaded_img_path)
        print(filteredNames)

        return render_template('index.html', query_path=uploaded_img_path, scores=path, names=filteredNames)
